...
class RegistrationActivationTests(TestCase):
    ...
    def test_register_user_success(self, mock_send_mail):
        ...
        self.assertIn("Please confirm your email address.",
    ...
    def test_register_password_mismatch(self):
        ...
        self.assertIn(
            "Bitte überprüfe deine Eingaben und versuche es erneut.", response.data)
    ...
    def test_activate_account_success(self):
        user = User.objects.create_user(
            email='activate@example.com', password='pass')
        ...
        self.assertIn("Account activated successfully",
                      response.data["message"])
    ...
    def test_activate_account_already_active(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            "Bitte überprüfe deine Eingaben und versuche es erneut.", response.data["message"])

    def test_activate_account_invalid_uid(self):
        activate_url = reverse('activate', kwargs={
                               'uidb64': 'invaliduid', 'token': 'sometoken'})
        response = self.client.get(activate_url)
        self.assertEqual(response.status_code, 400)
        self.assertIn("Invalid or expired activation link",
                      response.data["message"])

    def test_activate_account_invalid_token(self):
        user = User.objects.create_user(
            email='badtoken@example.com', password='pass')
        user.is_active = False
        user.save()
    ...
    def test_password_mismatch(self):
        ...
        self.assertEqual(
            serializer.errors["password"][0], "Passwords do not match.")
